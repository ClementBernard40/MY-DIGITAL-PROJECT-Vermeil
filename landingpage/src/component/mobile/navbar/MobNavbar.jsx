...
function MobNavbar() {
  ...
  return (
    <nav
      className={`p-4 flex items-center z-40 justify-between ${
    ...
    >
      ...
      >
        {(ref) => (
          <div
            ref={ref}
            className={`absolute top-16 right-4 mt-2 w-56 bg-white divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-10 ${
              isOpen ? "block" : "hidden"
            }`}
          >
            <div className="py-1">
              <a
                href={isSubPage ? "/#" : "/#"}
                className="Rubik block px-4 py-2 text-sm text-darkgreen hover:bg-gray-100"
